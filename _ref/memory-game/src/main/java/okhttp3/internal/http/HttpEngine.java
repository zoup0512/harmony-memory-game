...
*/
    ...
    private HttpStream connect() throws RouteException, RequestException, IOException {
        return this.streamAllocation.newStream(this.client.connectTimeoutMillis(), this.client.readTimeoutMillis(), this.client.writeTimeoutMillis(), this.client.retryOnConnectionFailure(), !this.networkRequest.method().equals(HttpRequest.METHOD_GET));
    }
    ...
    public static boolean hasBody(Response response) {
        ...
        if ((responseCode < 100 || responseCode >= 200) && responseCode != AppLovinErrorCodes.NO_FILL && responseCode != 304) {
            return true;
        }
    ...
    private Request networkRequest(Request request) throws IOException {
        ...
        if (request.header(HttpHeader.HOST) == null) {
            result.header(HttpHeader.HOST, Util.hostHeader(request.url(), false));
        }
    ...
    private static Headers combine(Headers cachedHeaders, Headers networkHeaders) throws IOException {
        ...
        for (i = 0; i < size; i++) {
            ...
            if (!("Warning".equalsIgnoreCase(fieldName) && value.startsWith(AppEventsConstants.EVENT_PARAM_VALUE_YES)) && (!OkHeaders.isEndToEnd(fieldName) || networkHeaders.get(fieldName) == null)) {
