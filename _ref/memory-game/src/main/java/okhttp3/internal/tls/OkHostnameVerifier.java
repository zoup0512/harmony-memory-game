...
public final class OkHostnameVerifier implements HostnameVerifier {
    ...
    private boolean verifyHostname(String hostname, String pattern) {
        if (hostname == null || hostname.length() == 0 || hostname.startsWith(FileUtils.HIDDEN_PREFIX) || hostname.endsWith("..") || pattern == null || pattern.length() == 0 || pattern.startsWith(FileUtils.HIDDEN_PREFIX) || pattern.endsWith("..")) {
            return false;
        }
        if (!hostname.endsWith(FileUtils.HIDDEN_PREFIX)) {
            hostname = hostname + '.';
        }
        if (!pattern.endsWith(FileUtils.HIDDEN_PREFIX)) {
            pattern = pattern + '.';
        }
