...
class NotificationCompatImplBase {
    ...
    public static int calculateTopPadding(Context ctx) {
        float largeFactor = (constrain(ctx.getResources().getConfiguration().fontScale, DefaultRetryPolicy.DEFAULT_BACKOFF_MULT, 1.3f) - DefaultRetryPolicy.DEFAULT_BACKOFF_MULT) / 0.29999995f;
        return Math.round(((DefaultRetryPolicy.DEFAULT_BACKOFF_MULT - largeFactor) * ((float) ctx.getResources().getDimensionPixelSize(R.dimen.notification_top_pad))) + (((float) ctx.getResources().getDimensionPixelSize(R.dimen.notification_top_pad_large_text)) * largeFactor));
    }
