...
final class BackStackRecord extends FragmentTransaction implements BackStackEntry, Runnable {
    ...
    private boolean configureTransitions(int containerId, TransitionState state, boolean isBack, SparseArray<Fragment> firstOutFragments, SparseArray<Fragment> lastInFragments) {
        ...
        if (transition != null) {
            FragmentTransitionCompat21.addTransitionTargets(enterTransition, sharedElementTransition, exitTransition, sceneRoot, anonymousClass1, state.nonExistentView, state.enteringEpicenterView, state.nameOverrides, enteringViews, exitingViews, namedViews, renamedViews, sharedElementTargets);
