...
class ValueAnimatorCompatImplGingerbread extends Impl {
    ...
    final void update() {
        if (this.mIsRunning) {
            float linearFraction = MathUtils.constrain(((float) (SystemClock.uptimeMillis() - this.mStartTime)) / ((float) this.mDuration), 0.0f, (float) DefaultRetryPolicy.DEFAULT_BACKOFF_MULT);
            if (this.mInterpolator != null) {
