...
final class ServiceMethod<T> {
    ...
    static final class Builder<T> {
        ...
        private ParameterHandler<?> parseParameterAnnotation(int p, Type type, Annotation[] annotations, Annotation annotation) {
            if (annotation instanceof Url) {
                ...
                if (this.isMultipart) {
                    ...
                    rawParameterType = Utils.getRawType(type);
                    if (!partName.isEmpty()) {
                        Headers headers = Headers.of(com.amazonaws.services.s3.Headers.CONTENT_DISPOSITION, "form-data; name=\"" + partName + "\"", "Content-Transfer-Encoding", part.encoding());
                        if (Iterable.class.isAssignableFrom(rawParameterType)) {
                            if (type instanceof ParameterizedType) {
        ...
        private RuntimeException methodError(Throwable cause, String message, Object... args) {
            return new IllegalArgumentException(String.format(message, args) + "\n    for method " + this.method.getDeclaringClass().getSimpleName() + FileUtils.HIDDEN_PREFIX + this.method.getName(), cause);
        }
