...
public class RectangularGrid extends GridLayout implements GameGrid {
    ...
    private class SuccessCellClickListener implements OnCheckedChangeListener {
        ...
        public void onCheckedChanged(CompoundButton buttonView, boolean isChecked) {
            ...
            RectangularGrid.this.currentSuccessCellsClicked = RectangularGrid.this.currentSuccessCellsClicked + 1;
            int indexOfChild = RectangularGrid.this.indexOfChild(buttonView);
    ...
    private class WrongCellListener implements OnCheckedChangeListener {
        ...
        public void onCheckedChanged(CompoundButton buttonView, boolean isChecked) {
            for (int i = 0; i < RectangularGrid.this.getChildCount(); i++) {
                ToggleButton button = (ToggleButton) RectangularGrid.this.getChildAt(i);
    ...
    public RectangularGrid(Context context, int colCount, int rowCount, int successCells, int width, int height) {
        super(context);
    ...
    public void rotateGrid(final int angle, int duration, View thumb, final RotationCompletedListener rotationCompletedListener) {
        ...
        ViewAnimator.animate(thumb).rotation((float) angle).duration((long) duration).onStop(new Stop() {
            public void onStop() {
                RectangularGrid.this.setRotation((float) angle);
                for (int i = 0; i < RectangularGrid.this.getChildCount(); i++) {
                    View view = RectangularGrid.this.getChildAt(i);
                    if (RectangularGrid.this.rotatedAngle != 0) {
                        view.setRotation((float) (360 - RectangularGrid.this.rotatedAngle));
                    }
                }
