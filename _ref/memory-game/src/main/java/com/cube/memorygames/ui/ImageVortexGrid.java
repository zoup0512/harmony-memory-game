...
public class ImageVortexGrid extends GridLayout implements GameGrid {
    ...
    private class SuccessCellClickListener implements OnCheckedChangeListener {
        ...
        public void onCheckedChanged(CompoundButton buttonView, boolean isChecked) {
            ...
            ImageVortexGrid.this.currentSuccessCellsClicked = ImageVortexGrid.this.currentSuccessCellsClicked + 1;
            int indexOfChild = ImageVortexGrid.this.indexOfChild(buttonView);
            if (ImageVortexGrid.this.currentSuccessCellsClicked >= ImageVortexGrid.this.successCells) {
                buttonView.setBackgroundResource(R.drawable.drawable_state_cell_success_last);
            }
            if (ImageVortexGrid.this.cellTypes == 0) {
                if (ImageVortexGrid.this.lastAddedSuccessCellIndex != -1) {
                    if (ImageVortexGrid.this.lastAddedSuccessCellIndex == indexOfChild) {
                        buttonView.setBackgroundResource(R.drawable.drawable_state_cell_success_last);
                        buttonView.setTag("ChangedBackgroud");
                    } else {
                        buttonView.setBackgroundResource(R.drawable.drawable_state_cell_wrong);
                        ImageVortexGrid.this.getChildAt(ImageVortexGrid.this.lastAddedSuccessCellIndex).setBackgroundResource(R.drawable.drawable_state_cell_unclicked);
                    }
                }
            } else if (ImageVortexGrid.this.cellTypes == 1) {
                buttonView.setTag("SuccessClicked");
            }
            ImageVortexGrid.this.notifySuccessCellClicked(indexOfChild);
        }
    }
    ...
    private class WrongCellListener implements OnCheckedChangeListener {
        ...
        public void onCheckedChanged(CompoundButton buttonView, boolean isChecked) {
            for (int i = 0; i < ImageVortexGrid.this.getChildCount(); i++) {
                ToggleButton button = (ToggleButton) ImageVortexGrid.this.getChildAt(i);
            ...
            buttonView.setEnabled(false);
            ImageVortexGrid.this.notifyFailCellClicked();
        }
    }

    public ImageVortexGrid(Context context, int colCount, int rowCount, int successCells, int width, int height) {
        super(context);
    ...
    public void animateCells() {
        for (int i = 0; i < this.colCount; i++) {
            for (int j = 0; j < this.rowCount; j++) {
                ...
                ViewAnimator.animate(view).scale(0.0f, DefaultRetryPolicy.DEFAULT_BACKOFF_MULT).duration(300).start();
            }
        }
    }
    ...
    public void animateFinishCells() {
        for (int i = 0; i < this.colCount; i++) {
            for (int j = 0; j < this.rowCount; j++) {
                View view = this.buttons[j][i];
                ViewAnimator.animate(view).scale(DefaultRetryPolicy.DEFAULT_BACKOFF_MULT, 0.0f).duration(200).start();
            }
        }
    }
    ...
    public void prepareForAnimation() {
        for (int i = 0; i < this.colCount; i++) {
            for (int j = 0; j < this.rowCount; j++) {
                this.buttons[j][i].setScaleX(0.0f);
                this.buttons[j][i].setScaleY(0.0f);
            }
        }
    }
    ...
    public void rotateGrid(final int angle, int duration, View thumb, final RotationCompletedListener rotationCompletedListener) {
        ...
        ViewAnimator.animate(thumb).rotation((float) angle).duration((long) duration).onStop(new Stop() {
            public void onStop() {
                ImageVortexGrid.this.setRotation((float) angle);
                for (int i = 0; i < ImageVortexGrid.this.getChildCount(); i++) {
                    View view = ImageVortexGrid.this.getChildAt(i);
                    if (ImageVortexGrid.this.rotatedAngle != 0) {
                        view.setRotation((float) (360 - ImageVortexGrid.this.rotatedAngle));
                    }
                }
