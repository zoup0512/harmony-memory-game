...
public class CountAllGrid extends GridLayout implements GameGrid {
    ...
    private class SuccessCellClickListener implements OnCheckedChangeListener {
        ...
        public void onCheckedChanged(CompoundButton buttonView, boolean isChecked) {
            ...
            CountAllGrid.this.currentSuccessCellsClicked = CountAllGrid.this.currentSuccessCellsClicked + 1;
            int indexOfChild = CountAllGrid.this.indexOfChild(buttonView);
            if (CountAllGrid.this.currentSuccessCellsClicked >= CountAllGrid.this.successCells) {
                buttonView.setBackgroundResource(R.drawable.drawable_state_cell_success_last);
            }
            if (CountAllGrid.this.cellTypes == 0) {
                if (CountAllGrid.this.lastAddedSuccessCellIndex != -1) {
                    if (CountAllGrid.this.lastAddedSuccessCellIndex == indexOfChild) {
                        buttonView.setBackgroundResource(R.drawable.drawable_state_cell_success_last);
                        buttonView.setTag("ChangedBackgroud");
                    } else {
                        buttonView.setBackgroundResource(R.drawable.drawable_state_cell_wrong);
                        CountAllGrid.this.getChildAt(CountAllGrid.this.lastAddedSuccessCellIndex).setBackgroundResource(R.drawable.drawable_state_cell_unclicked);
                    }
                }
            } else if (CountAllGrid.this.cellTypes == 1) {
                buttonView.setTag("SuccessClicked");
            }
            CountAllGrid.this.notifySuccessCellClicked(indexOfChild);
        }
    }
    ...
    private class WrongCellListener implements OnCheckedChangeListener {
        ...
        public void onCheckedChanged(CompoundButton buttonView, boolean isChecked) {
            for (int i = 0; i < CountAllGrid.this.getChildCount(); i++) {
                ToggleButton button = (ToggleButton) CountAllGrid.this.getChildAt(i);
            ...
            buttonView.setEnabled(false);
            CountAllGrid.this.notifyFailCellClicked();
        }
    }

    public CountAllGrid(Context context, int colCount, int rowCount, int successCells, int width, int height) {
        super(context);
    ...
    public void hideChallengeCells() {
        for (int i = 0; i < getChildCount(); i++) {
            ...
            ViewAnimator.animate(button).scale(DefaultRetryPolicy.DEFAULT_BACKOFF_MULT, 0.0f).duration(200).start();
        }
    }
    ...
    public void rotateGrid(final int angle, int duration, View thumb, final RotationCompletedListener rotationCompletedListener) {
        ...
        ViewAnimator.animate(thumb).rotation((float) angle).duration((long) duration).onStop(new Stop() {
            public void onStop() {
                CountAllGrid.this.setRotation((float) angle);
                for (int i = 0; i < CountAllGrid.this.getChildCount(); i++) {
                    View view = CountAllGrid.this.getChildAt(i);
                    if (CountAllGrid.this.rotatedAngle != 0) {
                        view.setRotation((float) (360 - CountAllGrid.this.rotatedAngle));
                    }
                }
    ...
    public void clearWrongCells() {
        ...
        ViewAnimator.animate(last).scale(DefaultRetryPolicy.DEFAULT_BACKOFF_MULT, 0.0f).duration(200).start();
