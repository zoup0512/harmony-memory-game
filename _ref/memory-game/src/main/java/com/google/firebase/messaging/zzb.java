...
class zzb {
    ...
    private static void zzc(Context context, String str, Intent intent) {
        ...
        AppMeasurement zzes = zzes(context);
        if (zzes != null) {
            zzes.zzd("fcm", str, bundle);
        } else {
            Log.w("FirebaseMessaging", "Unable to log event: analytics library is missing");
        }
    }

    private static AppMeasurement zzes(Context context) {
        if (null != null) {
            return aOm;
        }
        try {
