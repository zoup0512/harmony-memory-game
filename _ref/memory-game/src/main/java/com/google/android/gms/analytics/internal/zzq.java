...
abstract class zzq<T extends zzp> extends zzc {
    ...
    public zzq(zzf com_google_android_gms_analytics_internal_zzf, zza<T> com_google_android_gms_analytics_internal_zzq_zza_T) {
        ...
        this.zzcza = com_google_android_gms_analytics_internal_zzq_zza_T;
    }
    ...
    private T zza(XmlResourceParser xmlResourceParser) {
            ...
            while (eventType != 1) {
                if (xmlResourceParser.getEventType() == 2) {
                    ...
                    if (toLowerCase.equals("screenname")) {
                        ...
                        if (!(TextUtils.isEmpty(attributeValue) || TextUtils.isEmpty(trim))) {
                            try {
                                this.zzcza.zzg(attributeValue, Boolean.parseBoolean(trim));
                        ...
                        if (!(TextUtils.isEmpty(attributeValue) || TextUtils.isEmpty(trim))) {
                            try {
                                this.zzcza.zzc(attributeValue, Integer.parseInt(trim));
        ...
        return this.zzcza.zzabb();
    }

    public T zzbx(int i) {
        try {
            return zza(zzyu().zzzi().getResources().getXml(i));
