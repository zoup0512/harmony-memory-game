...
public final class zzanv {
    private static final Map<Class<?>, Class<?>> bfl;
    private static final Map<Class<?>, Class<?>> bfm;
    ...
    static {
        ...
        bfl = Collections.unmodifiableMap(hashMap);
        bfm = Collections.unmodifiableMap(hashMap2);
    }
    ...
    public static boolean zzk(Type type) {
        return bfl.containsKey(type);
    }

    public static <T> Class<T> zzp(Class<T> cls) {
        Class<T> cls2 = (Class) bfl.get(zzann.zzy(cls));
