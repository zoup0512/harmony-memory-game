...
public class VastIconXmlManager {
    ...
    Integer getOffsetMS() {
        Integer num = null;
        try {
            num = Strings.parseAbsoluteOffset(XmlUtils.getAttributeValue(this.mIconNode, OFFSET));
        } catch (NumberFormatException e) {
            MoPubLog.d(String.format("Invalid VAST icon offset format: %s:", new Object[]{r1}));
        }
        return num;
    }
    ...
    Integer getDurationMS() {
        Integer num = null;
        try {
            num = Strings.parseAbsoluteOffset(XmlUtils.getAttributeValue(this.mIconNode, DURATION));
        } catch (NumberFormatException e) {
            MoPubLog.d(String.format("Invalid VAST icon duration format: %s:", new Object[]{r1}));
        }
        return num;
    }
