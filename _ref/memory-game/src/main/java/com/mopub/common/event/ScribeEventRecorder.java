        ...
        if (!this.mScribeRequestManager.isAtCapacity()) {
            ...
            if (!dequeueEvents.isEmpty()) {
                this.mScribeRequestManager.makeRequest(new ScribeRequest$ScribeRequestFactory() {
                    public ScribeRequest createRequest(ScribeRequest$Listener scribeRequest$Listener) {
                        return new ScribeRequest(ScribeEventRecorder.SCRIBE_URL, dequeueEvents, ScribeEventRecorder.this.mEventSerializer, scribeRequest$Listener);
                    }
