...
public class LineDataSet extends LineRadarDataSet<Entry> implements ILineDataSet {
    ...
    private float mCircleRadius = CloseButton.STROKE_WIDTH;
    ...
    public void setCubicIntensity(float intensity) {
        if (intensity > DefaultRetryPolicy.DEFAULT_BACKOFF_MULT) {
            intensity = DefaultRetryPolicy.DEFAULT_BACKOFF_MULT;
        }
        if (intensity < 0.05f) {
            intensity = 0.05f;
        }
