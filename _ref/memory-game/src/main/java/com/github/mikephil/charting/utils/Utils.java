...
public abstract class Utils {
    ...
    public static String formatNumber(float number, int digitCount, boolean separateThousands, char separateChar) {
        ...
        boolean zero = false;
        if (number < DefaultRetryPolicy.DEFAULT_BACKOFF_MULT && number > -1.0f) {
            zero = true;
        }
        if (number < 0.0f) {
            neg = true;
    ...
    public static void drawMultilineText(Canvas c, StaticLayout textLayout, float x, float y, TextPaint paint, MPPointF anchor, float angleDegrees) {
        ...
        paint.setTextAlign(Align.LEFT);
        if (angleDegrees != 0.0f) {
            drawOffsetX -= 0.5f * drawWidth;
            drawOffsetY -= 0.5f * drawHeight;
            float translateX = x;
            float translateY = y;
            if (!(anchor.x == 0.5f && anchor.y == 0.5f)) {
    ...
    public static void drawMultilineText(Canvas c, String text, float x, float y, TextPaint paint, FSize constrainedToSize, MPPointF anchor, float angleDegrees) {
        drawMultilineText(c, new StaticLayout(text, 0, text.length(), paint, (int) Math.max(Math.ceil((double) constrainedToSize.width), 1.0d), Alignment.ALIGN_NORMAL, DefaultRetryPolicy.DEFAULT_BACKOFF_MULT, 0.0f, false), x, y, paint, anchor, angleDegrees);
    }
