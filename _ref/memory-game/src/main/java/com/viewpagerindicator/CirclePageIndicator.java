...
public class CirclePageIndicator extends View implements PageIndicator {
    ...
    private int measureLong(int measureSpec) {
        ...
        int count = this.mViewPager.getAdapter().getCount();
        int result = (int) (((((float) (getPaddingLeft() + getPaddingRight())) + (((float) (count * 2)) * this.mRadius)) + (((float) (count - 1)) * this.mRadius)) + DefaultRetryPolicy.DEFAULT_BACKOFF_MULT);
    ...
    private int measureShort(int measureSpec) {
        ...
        int result = (int) ((((2.0f * this.mRadius) + ((float) getPaddingTop())) + ((float) getPaddingBottom())) + DefaultRetryPolicy.DEFAULT_BACKOFF_MULT);
        if (specMode == Integer.MIN_VALUE) {
            return Math.min(result, specSize);
        }
        return result;
