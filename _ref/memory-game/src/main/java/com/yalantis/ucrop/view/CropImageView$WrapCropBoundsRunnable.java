...
class CropImageView$WrapCropBoundsRunnable implements Runnable {
    ...
    public CropImageView$WrapCropBoundsRunnable(CropImageView cropImageView, long durationMs, float oldX, float oldY, float centerDiffX, float centerDiffY, float oldScale, float deltaScale, boolean willBeImageInBoundsAfterTranslate) {
        ...
        if (cropImageView != null) {
            ...
            if (currentMs < ((float) this.mDurationMs)) {
                ...
                if (!this.mWillBeImageInBoundsAfterTranslate) {
                    cropImageView.zoomInImage(this.mOldScale + newScale, CropImageView.access$000(cropImageView).centerX(), CropImageView.access$000(cropImageView).centerY());
                }
