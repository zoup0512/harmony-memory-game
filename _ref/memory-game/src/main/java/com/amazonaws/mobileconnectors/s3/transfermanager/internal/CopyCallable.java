...
public class CopyCallable implements Callable<CopyResult> {
    ...
    public CopyResult call() throws Exception {
        this.copy.setState(TransferState.InProgress);
        if (!isMultipartCopy()) {
    ...
    private void populateMetadataWithEncryptionParams(ObjectMetadata source, ObjectMetadata destination) {
        ...
        String[] headersToCopy = new String[]{Headers.CRYPTO_CEK_ALGORITHM, Headers.CRYPTO_IV, Headers.CRYPTO_KEY, Headers.CRYPTO_KEY_V2, Headers.CRYPTO_KEYWRAP_ALGORITHM, Headers.CRYPTO_TAG_LENGTH, Headers.MATERIALS_DESCRIPTION, Headers.UNENCRYPTED_CONTENT_LENGTH, Headers.UNENCRYPTED_CONTENT_MD5};
        if (userMetadataSource != null) {
            ...
            int length = headersToCopy.length;
            while (i < length) {
                String header = headersToCopy[i];
