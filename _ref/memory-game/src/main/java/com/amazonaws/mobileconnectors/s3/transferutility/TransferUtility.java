...
public class TransferUtility {
    ...
    public List<TransferObserver> getTransfersWithType(TransferType type) {
        ...
        while (c.moveToNext()) {
            try {
                transferObservers.add(new TransferObserver(c.getInt(c.getColumnIndexOrThrow(TransferTable.COLUMN_ID)), this.dbUtil, c));
            } finally {
                c.close();
            }
        }
    ...
    public void pauseAllWithType(TransferType type) {
        ...
        while (c.moveToNext()) {
            try {
                pause(c.getInt(c.getColumnIndexOrThrow(TransferTable.COLUMN_ID)));
            } finally {
                c.close();
            }
        }
    ...
    public void cancelAllWithType(TransferType type) {
        ...
        while (c.moveToNext()) {
            try {
                cancel(c.getInt(c.getColumnIndexOrThrow(TransferTable.COLUMN_ID)));
            } finally {
                c.close();
            }
        }
