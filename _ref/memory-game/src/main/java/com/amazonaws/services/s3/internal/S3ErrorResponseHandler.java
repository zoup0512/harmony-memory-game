...
public class S3ErrorResponseHandler implements HttpResponseHandler<AmazonServiceException> {
    ...
    public AmazonServiceException handle(HttpResponse errorResponse) throws IOException {
        ...
        try {
            ...
            try {
                Document document = XpathUtils.documentFrom(str);
                ...
                AmazonServiceException ase = new AmazonS3Exception(message);
                ...
                ase.setCloudFrontId((String) errorResponse.getHeaders().get(Headers.CLOUD_FRONT_ID));
                return ase;
            } catch (Exception ex) {
                if (log.isDebugEnabled()) {
                    log.debug("Failed in parsing the response as XML: " + str, ex);
                }
                return newAmazonS3Exception(str, errorResponse);
            }
    ...
    private AmazonS3Exception newAmazonS3Exception(String errmsg, HttpResponse httpResponse) {
        ...
        ase.setRequestId((String) headers.get(Headers.REQUEST_ID));
        ase.setExtendedRequestId((String) headers.get(Headers.EXTENDED_REQUEST_ID));
        ase.setCloudFrontId((String) headers.get(Headers.CLOUD_FRONT_ID));
        return ase;
    }
