...
public class RegionMetadataParser {
    ...
    private static void addRegionEndpoint(Region region, Element endpointElement, boolean endpointVerification) {
        ...
        if (!endpointVerification || verifyLegacyEndpoint(hostname)) {
            ...
            region.getHttpSupport().put(serviceName, Boolean.valueOf(ServerProtocol.DIALOG_RETURN_SCOPES_TRUE.equals(http)));
            region.getHttpsSupport().put(serviceName, Boolean.valueOf(ServerProtocol.DIALOG_RETURN_SCOPES_TRUE.equals(https)));
            return;
        }
