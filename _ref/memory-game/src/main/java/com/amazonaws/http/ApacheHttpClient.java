...
public class ApacheHttpClient implements HttpClient {
    ...
    public ApacheHttpClient(ClientConfiguration config) {
        ...
        ((SSLSocketFactory) this.httpClient.getConnectionManager().getSchemeRegistry().getScheme(Constants.HTTPS).getSocketFactory()).setHostnameVerifier(SSLSocketFactory.BROWSER_COMPATIBLE_HOSTNAME_VERIFIER);
    }
    ...
    private HttpUriRequest createHttpRequest(HttpRequest request) {
        ...
        if (method.equals(HttpRequest.METHOD_POST)) {
            ...
            Object httpRequest2 = putRequest;
        } else if (method.equals(HttpRequest.METHOD_DELETE)) {
